...
int main() {
	char note1, note2, dummy; // note names and dummy char
	int pc1, pc2, interval; // pitch classes and interval

	// get inputs
	printf("Please enter two natural tones.\nFirst note: ");
	scanf("%c%c", &note1, &dummy);
	printf("Second note: ");
	scanf("%c", &note2);

	// translate from note name to pitch class
	switch (note1) {
	case 'C': case 'c':
		pc1 = 0;
		break;
	...
	printf("\n%d semitones up or %d semitones down\n", interval, interval ? 12 - interval : 0);
